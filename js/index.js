...
async function initialData() {
  ...
  if (bgResponse.length > 0) {
    ...
    if (bgResponse[1] != null) {
      bgElement.css("background-image", 'url("./img/assets/' + bgResponse[1] + '")');
    } else {
  ...
  if (initialData.length > 0) {
    initialData.forEach(function (value, num) {
      ...
      image.onload = function () {
        theWheel.addSegment({ image: "./img/" + value[3], fillStyle: num % 2 == 0 ? "#f42d92" : "#5dc1d8", text: value[2], imgData: image, imageDirection: "S", strokeStyle: "black" });
        theWheel.draw();
      };
    });
